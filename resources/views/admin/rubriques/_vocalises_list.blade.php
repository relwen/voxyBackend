@if($rubrique->directSections->isEmpty())
    <div class="py-24 text-center bg-white rounded-[3rem] border-2 border-dashed border-gray-100">
        <div class="w-24 h-24 bg-gray-50 rounded-full flex items-center justify-center mx-auto mb-6 shadow-inner">
            <i class="fas fa-microphone text-gray-200 text-4xl"></i>
        </div>
        <h3 class="text-gray-400 font-bold text-xl">Aucune vocalise enregistrée</h3>
        <p class="text-gray-300 text-sm mt-1">Commencez par ajouter une série de vocalises.</p>
            ...
            <div class="group relative bg-white rounded-[2.5rem] p-8 shadow-xl shadow-slate-200/40 border border-gray-100 hover:shadow-2xl hover:shadow-primary/10 transition-all overflow-hidden">
                ...
                <div class="relative items-start justify-between flex mb-6">
                    ...
                    <div class="flex gap-2 opacity-0 group-hover:opacity-100 transition-all translate-y-2 group-hover:translate-y-0">
                        <button @click="editMesse({{ $vocaliseSection->id }})" class="w-10 h-10 rounded-xl bg-blue-50 text-blue-600 flex items-center justify-center hover:bg-blue-100">
                            <i class="fas fa-edit text-sm"></i>
                        </button>
                        <button @click="deleteMesse({{ $vocaliseSection->id }})" class="w-10 h-10 rounded-xl bg-red-50 text-red-600 flex items-center justify-center hover:bg-red-100">
                            <i class="fas fa-trash text-sm"></i>
                        </button>
                    </div>
                </div>

                <h3 class="text-2xl font-black text-gray-900 leading-tight mb-2">{{ $vocaliseSection->nom }}</h3>
                <p class="text-gray-400 text-xs font-black uppercase tracking-widest">{{ $vocaliseSection->vocalises->count() }} VOCALISES</p>
