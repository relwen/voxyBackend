...
@section('title', 'Modifier la Chorale - VoXY Admin')
@section('page-title', 'Modifier Chorale')

@section('content')
<div class="max-w-4xl mx-auto">
    <div class="mb-8 flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Modifier {{ $chorale->name }}</h1>
            <p class="text-gray-500 mt-1">Mettez à jour les informations de cette chorale.</p>
        </div>
    ...
    <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
        <form action="{{ route('admin.chorales.update', $chorale->id) }}" method="POST" class="p-8 md:p-10 space-y-8">
            @csrf
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <textarea name="description" id="description" rows="5"
                                  ...
                                  placeholder="Brève description de la chorale...">{{ old('description', $chorale->description) }}</textarea>
                        @error('description')
                            <p class="mt-2 text-xs text-red-600 font-medium">{{ $message }}</p>
                        @enderror
                    </div>
