...
<head>
    ...
    <title>Modifier la Référence - VoXY Admin</title>
    ...
    <div class="max-w-2xl mx-auto py-6 sm:px-6 lg:px-8">
        <div class="bg-white shadow overflow-hidden sm:rounded-lg">
            <div class="px-4 py-5 sm:px-6">
                <h3 class="text-lg leading-6 font-medium text-gray-900">Modifier la section</h3>
                <p class="mt-1 max-w-2xl text-sm text-gray-500">Modifiez les informations de la section "{{ $reference->name }}"</p>
            </div>

            <form method="POST" action="{{ route('admin.references.update', $reference->id) }}" class="px-4 py-5 sm:p-6">
                @csrf
                ...
                <div class="mb-6">
                    ...
                    <select name="messe_id" id="messe_id" required
                        ...
                        @foreach($messes as $messe)
                            <option value="{{ $messe->id }}" {{ old('messe_id', $reference->messe_id) == $messe->id ? 'selected' : '' }}>
                                {{ $messe->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $reference->name) }}" required
                ...
                <div class="mb-6">
                    ...
                    <textarea name="description" id="description" rows="3"
                              ...
                              placeholder="Description de cette section...">{{ old('description', $reference->description) }}</textarea>
                    @error('description')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-6">
                    ...
                    <input type="number" name="order_position" id="order_position" value="{{ old('order_position', $reference->order_position) }}" min="1"
