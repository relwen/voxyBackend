...
<head>
    ...
    <title>Créer une Messe - VoXY Admin</title>
    ...
    <div class="max-w-2xl mx-auto py-6 sm:px-6 lg:px-8">
        <div class="bg-white shadow overflow-hidden sm:rounded-lg">
            <div class="px-4 py-5 sm:px-6">
                <h3 class="text-lg leading-6 font-medium text-gray-900">Créer une nouvelle messe</h3>
                <p class="mt-1 max-w-2xl text-sm text-gray-500">Ajoutez une nouvelle messe avec ses sections (Kyrié, Gloria, etc.)</p>
            </div>

            <form method="POST" action="{{ route('admin.messes.store') }}" class="px-4 py-5 sm:p-6">
                @csrf
                ...
                <div class="mb-6">
                    ...
                    <textarea name="description" id="description" rows="3"
                              ...
                              placeholder="Description de cette messe...">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-6">
                    ...
                    <div class="mt-1 flex items-center space-x-3">
                        <input type="color" name="couleur" id="couleur" value="{{ old('couleur', '#8B5CF6') }}"
                               class="h-10 w-20 border border-gray-300 rounded-md cursor-pointer @error('couleur') border-red-300 @enderror">
                        <input type="text" value="{{ old('couleur', '#8B5CF6') }}" 
